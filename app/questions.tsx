...
const Questions = () => {
  ...
  const [answers, setAnswers] = React.useState<
    { questionId: number; answer: any; correct: boolean }[]
  >([]);
    ...
    const fetchQuestions = async () => {
      try {
        const response = await fetch(
          `http://${process.env.EXPO_PUBLIC_SERVER_IP_ADDRESS}:8081/questions/fetch?testId=${testId}`,
          {
            method: "GET",
            headers: {
  ...
  const handleStart = () => {
    if (agreed) {
      ...
      Alert.alert(
        "Error",
        "You must agree to the instructions before starting the test."
      );
    }
  };
  ...
  const sendResultsToBackend = async (finalAnswers: any[]) => {
    ...
    try {
      const response = await fetch(
        `http://${process.env.EXPO_PUBLIC_SERVER_IP_ADDRESS}:8081/marks/submit`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <View style={styles.questionContainer}>
        ...
        >
          <Card
            ...
            onPress={() => setSelectedOption(question.option1)}
          >
          ...
          <Card
            style={styles.optionCard}
            onPress={() => setSelectedOption(question.option2)}
          >
            <Card.Content style={styles.optionContent}>
          ...
          <Card
            style={styles.optionCard}
            onPress={() => setSelectedOption(question.option3)}
          >
            <Card.Content style={styles.optionContent}>
          ...
          <Card
            style={styles.optionCard}
            onPress={() => setSelectedOption(question.option4)}
          >
            <Card.Content style={styles.optionContent}>
  ...
  const renderResults = () => {
    ...
    const completeAnswers = allQuestions.map((questionIndex) => {
      ...
      return answer
        ? answer
        : { questionId: question.questionId, answer: "", correct: false };
    });
    ...
    return (
      <View>
        ...
        <Button
          mode="contained"
          onPress={handleBackToUpcomingTests}
